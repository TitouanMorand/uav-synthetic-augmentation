...
def expand_box(
    ...
    box_w = box.x2 - box.x1
    box_h = box.y2 - box.y1
    margin_x = int(round(pixel_margin + box_w * relative_margin))
    margin_y = int(round(pixel_margin + box_h * relative_margin))
...
def object_mask_from_labels(
    ...
    return object_mask_from_boxes(boxes, image_size, pixel_margin, relative_margin)
...
def background_inpaint_mask_from_labels(
    ...
    """Return a Diffusers inpaint mask: white background, black protected drone boxes."""
    object_mask = np.array(object_mask_from_labels(label_path, image_size, pixel_margin, relative_margin))
